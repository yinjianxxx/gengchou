...
下载 `ai-usage-monitor.exe`，放入任意当前用户可写的目录直接运行，无需安装器。
可执行文件目前未做代码签名，如有需要可用 Release 中的 `SHA256SUMS` 校验
（应用内更新会自动校验）。
...
- **右键单击**打开菜单：服务商、刷新间隔、通知、开机启动、位置、语言、
  更新检查、组件显示与退出。
...
数字为当前 5 小时用量；下方两条用量条分别表示 5 小时（上）和 7 天（下）
周期。暂无数据时数字替换为服务商首字母，接近上限时切换为警告色。上图由
