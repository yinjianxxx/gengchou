...
更筹 Gengchou（读作 `gēng chóu`）是一款轻量级原生 Windows 应用。“更筹”原指
古代夜间计时、报更使用的竹签，也可借指时间。应用把服务商实际返回的配额窗口
放进任务栏组件和紧凑托盘图标，查配额无需再打开服务商控制台。
本项目原名 AI Usage Monitor，更名后继续使用原设置与更新路径；最初派生自
...
- 跟随系统主题的详情弹窗，含各服务商状态、精确重置时间、实时刷新的轮询倒计时，
  并可在本次打开期间临时锁定位置
- 可选的长期浮窗，复用紧凑小组件布局，整窗可拖动、跨重启记忆位置，并可
  恢复到主显示器工作区右下角
...
- **右键单击**组件或托盘图标打开菜单，直接单击**图标**、**小组件**或**浮窗**
  即可切换。位置重置、通知和开机启动等位于**设置**。
- 展开**刷新**后，可单击顶部的**现在**立即刷新，也可选择自动刷新频率。

### 任务栏组件

<img src=".github/taskbar-widget.png" alt="嵌入 Windows 任务栏的更筹 Gengchou 组件">

组件直接嵌入任务栏。每家服务商使用内容自适应的单行徽章，显示 logo、额度窗口标签和
短窗口用量；其他窗口达到 90% 时由该警示窗口接管，徽章变红并追加倒计时。悬停徽章
可查看该服务商报告的全部额度窗口与重置时间。拖动左侧分隔线可调整位置；
拖到另一条任务栏可切换显示器。Explorer 暂时不可用时组件会保持隐藏，不会退回桌面
弹窗；任务栏恢复后自动重新嵌入。

### 浮窗

浮窗是与任务栏组件、详情弹窗相互独立的长期数字视图，不会自动充当任务栏组件的
回退界面，也不是任务栏徽章的拉宽副本。它在每个服务商 logo 旁保留最多两个用量
最高的配额窗口，将紧凑标签、百分比和倒计时对齐在各自微量表上方。可从整窗任意
位置拖动；短按仍会打开详情。浮窗始终位于普通窗口之上，位置会跨重启记忆，并以
8 个逻辑像素的安全间距保持在当前工作区内，也可在**设置**中恢复默认位置。两种
独立视图搭配单一中性托盘图标，可避免同一个数字在托盘重复出现。
...
自身渲染——运行 `.\gengchou.exe --dump-tray-icons .\preview` 可导出
全部服务商图标状态。
